module Handler.ChatExampleJs where
...
-- original code, requires conduits.  
--    (sourceWS $$ mapM_C (\msg ->
--      (liftIO . atomically) $ writeTChan writeChan $ name <> ": " <> msg))


getChatExampleJsR :: Handler Html
getChatExampleJsR = do
  webSockets chatApp
  defaultLayout $ do
    setTitle "chat test"
    [whamlet|
...
postChatExampleJsR :: Handler Html
postChatExampleJsR = error "Not yet implemented: postChatExampleJsR"
